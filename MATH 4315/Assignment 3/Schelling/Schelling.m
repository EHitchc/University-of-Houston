classdef Schelling    
    ...
    methods
        ...
        function update(obj)
            v = VideoWriter('record.avi');  %added
            v.FrameRate=10  %added
            open(v);  %added
            set(gca,'nextplot','replacechildren'); %added
            ...
            for i=2:1:obj.n_iterations
                ...
                hold off
                final=obj.agents;
                save('final_matrix','final');
                M=getframe;   %added
                writeVideo(v,M);   %added
                pause(0.01);
        ...
        function [count_similar, count_different] = similarity(obj, x, y)
            count_similar = zeros(x,y);
            count_different = zeros(x,y);
            for j=1:x
                for k=1:y
                    current_agent = obj.agents(j,k);
                    neighbours_index = obj.get_neighbours_index(j,k);
                    for i=1:1:length(neighbours_index)
                        if current_agent == obj.agents(neighbours_index(i));
